...
function handleFlip(id) {
  ...
  if (!isFlipped) {
    ...
    // Gatilho Secreto: Dispara pisca do logo Super Trunfo se estiver na faixa 0500-0509
    if (id >= "0500" && id <= "0509") {
...
// 14.2 — animarLogoSuperTrunfo — pisca o logo Super Trunfo no footer 3 vezes ao girar uma carta lendária/secreta
function animarLogoSuperTrunfo() {
  const trunfoImg = document.getElementById("superTrunfoLogo");
  if (!trunfoImg) return;

  console.log(
    "%c 🃏 [EASTER EGG] Card Secreto Super Trunfo Ativado! Piscando logo...",
    "color: #ffe600; font-weight: bold;",
  );

  trunfoImg.classList.remove("hidden");
  trunfoImg.classList.remove("opacity-100");
  trunfoImg.classList.add("opacity-0");

  let piscadas = 0;
  const maxPiscadas = 3;

  const cicloPisca = setInterval(() => {
    if (trunfoImg.classList.contains("opacity-0")) {
      trunfoImg.classList.remove("opacity-0");
      trunfoImg.classList.add("opacity-100");
    } else {
      trunfoImg.classList.remove("opacity-100");
      trunfoImg.classList.add("opacity-0");
      piscadas++;
      ...
      if (piscadas >= maxPiscadas) {
        clearInterval(cicloPisca);
        setTimeout(() => {
          trunfoImg.classList.add("hidden");
        }, 300);
      }
    }
  }, 700);
